...
[ -f $RCDIR/key-bindings.sh ] && source $RCDIR/key-bindings.sh
[ -f $RCDIR/mysql.bash/mysql-colorize.bash ] && source $RCDIR/mysql.bash/mysql-colorize.bash
...
function _update_ps1() {
	[ -f ~/.local/powerline-shell/powerline-shell.py ] && \
      export PS1="$(~/.local/powerline-shell/powerline-shell.py $? 2> /dev/null)"
}
